...
Template.registerHelper('pageheader', function() {
  if (Session.equals('initialized', true)) {
    return "Participatory Budgeting survey: Ward " + Session.get('ward_ID_value');
  } else {
    return "Participatory Budgeting survey"
  }
});



Template.registerHelper('experiment_finished', function() {
  return Session.get('experiment_finished');
});
...
Template.registerHelper('current_phase', function() {
  return Answers.findOne({
    user_id: Session.get("user_ID_value")
  }).current_phase;
});
